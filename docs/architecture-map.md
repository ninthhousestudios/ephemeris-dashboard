...
| `tabs/jpl_horizons/jpl_horizons_tab.dart` | `JplHorizonsTab` | Two-pane console (Row >900px, stacked below). Ephem-type chips gate per-type controls; the target field has a "Bodies" popup picker (`horizonsBodyGroups`); response pane renders a parsed table with a Table/Raw toggle + CSV export (`ExportService.matrixToCsv`/`saveText`) / api-error / disambiguation chips / SPK save-`.bsp` button (`ExportService.saveBytes`). |
