...
| 20 | "UTC offset is a derived projection of the linked zone" — CONTEXT §Time Zone, swe-dashboard/112 | c + d | CLAUDE.md invariant + tests (`test/timezone_offset_test.dart`, `test/context_utc_derivation_test.dart`) | routed → CLAUDE.md. Conditional purity (like Locked Flags): when `timeZoneId` is set the offset is a pure function of (zone, Context date); manual edit detaches. Not graph-expressible (a value-derivation rule, not a dependency), so it is a behavioral invariant pinned by tests: the resolver's confidence cases and the notifier pipe (link+derive+preserve-wall, detach, re-derive across DST, persistence round-trip, series non-perturbation). A `forbidden_pattern` was not considered — there is no import or identifier to ban; the risk is a *future* per-step or `ref.listen` re-derivation, which the tests guard by asserting series steps leave the offset untouched. |
